...
#				            playerFlapped[ind] = True
        ...
        display_map=np.zeros((12,9))	
         
        for pipe in range(len(lowerPipes)):
        	
        	lpipex=lowerPipes[pipe]['x']
        	lpipey=lowerPipes[pipe]['y']
        	
        	upipex=upperPipes[pipe]['x']
        	upipey=upperPipes[pipe]['y']+320
        	
        	if lpipex+52>0 and lpipex+52<288:
        		
        		lposx=int(lpipex/32)
        		lposy=int(lpipey/32)
        		display_map[lposy+1:13,lposx:lposx+1]=1
        		
        		uposx=int(upipex/32)
        		uposy=int(upipey/32)
        		display_map[0:uposy,uposx:uposx+1]=1
        ...
        for ind in range(len(population)):
			...
			if crashCheck[ind]==False:
				
				pposy=int(playery[ind]/32)
				pposx=int(playerx/32)
				
				copy=display_map[pposy,pposx]
				
		    	display_map[pposy,pposx]=pposy
		    	
		    	x = np.hstack(display_map[pposy,pposx:pposx+4])
		    	
		    	if crashCheck[ind]==False:
		    		pygame.draw.rect(SCREEN, (255,0,0), pygame.Rect(playerx, playery[ind], 96 , 2))
		    	

#		    	x = np.array([playery[ind], pipe_pos_x, pipe_pos_y])
		  
		    	
		    	display_map[pposy,pposx]=copy
        ...
        pygame.draw.rect(SCREEN, (0,0,0), pygame.Rect(0,0, 35, 60))
        for i in range(12):
        	for j in range(1,6):
        		if display_map[i,j]==1:
        			pygame.draw.rect(SCREEN, (255,255,255), pygame.Rect(j*5, i*5, 5, 5))	
	...
	if generation==0:
		...
		for j in range(population_size):
			...
			b2 = np.random.randn(1, nn_output_dim) / np.sqrt(nn_hdim1)
		   	model = { 'Generation': generation, 'Ind': j, 'Genome': {'W1': W1, 'b1': b1, 'W2': W2, 'b2': b2}, 'Fitness': 'und'}
		   	population+=[model]
		...
		for x in range(nr_new_inds):			
			...
			new_genome_mut = mutation(new_genome,mutation_prob)
			
			population += [{ 'Population': 1, 'Generation': generation, 'Ind': (size_pre_population)+x, 'Genome': new_genome_mut, 'Fitness': 'und'}]
